...
class Feature(SubModule):
    def __init__(self):
        ...
        # 2. 定义本地权重路径 (假设你的 .pth 文件放在项目根目录下的 models 文件夹中)
        pretrained_path = 'models/mobilenetv2_100_ra-b33bc2c4.pth'

        # 3. 手动加载权重
        if os.path.exists(pretrained_path):
            print(f"Loading MobileNetV2 backbone from local file: {pretrained_path}")
            # map_location='cpu' 保证即使没有 GPU 也能加载
            state_dict = torch.load(pretrained_path, map_location='cpu')

            # strict=False 是必须的！
            # 因为原始权重包含分类层(classifier)，而 features_only=True 的模型没有分类层，
            # strict=True 会因为键值不匹配而报错。
            model.load_state_dict(state_dict, strict=False)
        else:
            # 如果找不到文件，打印警告并报错（或者你可以选择在这里改为 pretrained=True 作为备选）
            raise FileNotFoundError(f"Local backbone weights not found at: {pretrained_path}")
        layers = [1,2,3,5,6]
