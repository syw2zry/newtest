...
# 1. 屏蔽 Rasterio 的 "NotGeoreferencedWarning"
warnings.filterwarnings("ignore", category=rasterio.errors.NotGeoreferencedWarning)

# 2. 屏蔽 PyTorch 的 "FutureWarning"
warnings.filterwarnings("ignore", category=FutureWarning)

# 3. 屏蔽 Scheduler 的 "UserWarning"
...
def sequence_loss(args, agg_preds, iter_preds, disp_gt, valid, loss_gamma=0.9):
    ...
    if ('kitti' in args.train_datasets) or ('eth3d' in args.train_datasets) or ('dfc2019' in args.train_datasets) or (
            'whu' in args.train_datasets):
        max_disp0 = 192
        max_disp1 = 192
        max_disp = 192
    else:
...
def fetch_optimizer(args, model):
    ...
    # 【新增逻辑】过拟合时使用死板的学习率，其他模式使用自适应的 OneCycleLR
    if getattr(args, 'exp_mode', 'full') == 'overfit':
        # StepLR 对于很短的步数相当于恒定学习率
...
def train(args):
    ...
    if exp_mode == 'overfit':
        args.num_steps = 500          # 只跑 500 步
        validation_frequency = 50     # 每 50 步看一次断崖下跌
    elif exp_mode == 'fast':
        args.num_steps = 20000        # 跑 2 万步
        validation_frequency = 200    # 每 200 步密集观察收敛曲线
    else:
        # args.num_steps 保持你在命令行传入的值 (默认 200000)
        validation_frequency = 1000   # 原版频率
    ...
    if args.restore_ckpt is not None:
        ...
        if match:
            ...
            # 为了让 scheduler 的学习率曲线也跟上步数（可选，避免恢复时学习率突变）
    ...
    while should_keep_training:
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            if total_steps % validation_frequency == validation_frequency - 1:
                ...
                elif 'dfc2019' in args.train_datasets:
                    results = validate_dfc2019(model.module, iters=args.valid_iters, args=args)
                ...
                elif 'kitti-epe' in results:
                    current_metric = results['kitti-epe']
                elif 'scene-disp-epe' in results:
                    current_metric = results['scene-disp-epe']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_datasets', default='sceneflow',
                        choices=['whu', 'dfc2019', 'sceneflow', 'kitti', 'middlebury_train', 'middlebury_finetune',
                                 'eth3d_train', 'eth3d_finetune'], help="training datasets.")
    ...
    # --- [核心修改]：智能路径拼接，防止套娃 ---
    # 如果 logdir 还没有以当前实验名结尾，才加上实验名，确保所有文件都在同一文件夹下
